...
export default {
  ...
  // 创建接口
  async create ({ path, name, method, reqContextType, reqData, resData, projectId, categoryId }) {
    // 获取链表尾
...
// 匹配接口
async function matchAPI ({ path, method, projectId }) {
  if (!matchsReady) {
    matchsReady = true
    const apis = await all(`SELECT * FROM api WHERE projectId = ${projectId}`)
    apis.forEach(api => {
